...
export default function ProjectSlicer({ projectId, onComplete }: ProjectSlicerProps) {
  ...
  const { woodworkingProjects, createProjectPlan, sliceProjectIntoLessons } = useUserProgressStore();
        ...
        <View style={styles.stepsContainer}>
          {slice.steps.map((step, index) => (
            <View key={index} style={styles.stepItem}>
              <Text style={[styles.stepNumber, { color: Colors[colorScheme ?? 'light'].tint }]}>
                {index + 1}
              </Text>
